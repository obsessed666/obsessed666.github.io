...
$(function() {
    ...
    $('#signupModal').on('show.bs.modal', function() {
        var mycaptcha = $('#signupModal').find('.row.captcha');
        mycaptcha.load($(this).find('form').attr('action') + ' .signuppage .wa-captcha', function () {});
    })
            ...
            $.post($.dsv.shop_url + 'cart/delete/', {html: 1, id: item.data('id')}, function (response) {
                item.animate({opacity: 0}, 'slow', function(){
                    if (response.data.count == 0) {
                        ...
                        $(this).closest('.flying-cart').fadeOut();
                    }
    ...
    $('#mailer-subscribe-form').submit(function() {
        ...
        var email_submit = form.find('button[type="submit"]');
        if (!email_input.val()) {
